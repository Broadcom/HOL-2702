...
def main(lsf=None, standalone=False, dry_run=False):
    ...
    # HOL-2702 specific check: modify VCF Automation VM CPU sizing if set to 32 CPUs
    if lsf.lab_sku and lsf.lab_sku.upper().strip() == 'HOL-2702':
        lsf.write_output('HOL-2702 lab SKU detected: checking VCF Automation VM CPU configuration...')
        ...
        if not lsf.sis:
            ...
            # Gather VCF Automation VM search patterns
            vravms = lsf.get_config_list('VCFFINAL', 'vravms') or lsf.get_config_list('VCF', 'vravms')
            vcfpostedgevms = lsf.get_config_list('VCF', 'vcfpostedgevms')
            
            search_patterns = []
            for entry_list in [vravms, vcfpostedgevms]:
                for entry in entry_list:
                    vm_pattern = entry.split(':')[0].strip()
                    if vm_pattern and vm_pattern not in search_patterns:
                        search_patterns.append(vm_pattern)
            
            default_patterns = ['^auto-platform.*', '^auto-a.*']
            for pat in default_patterns:
                if pat not in search_patterns:
            ...
            for pat in search_patterns:
                try:
                    matches = lsf.get_vm_match(pat)
                    for vm in matches:
                        vm_id = getattr(vm, '_moId', vm.name)
                        if vm_id not in seen_vms:
                            seen_vms.add(vm_id)
                            vcfa_vms.append(vm)
                except Exception as e:
                    lsf.write_output(f'Error matching pattern {pat}: {e}')
            
            if not vcfa_vms:
                lsf.write_output('No VCF Automation VM found in inventory')
            else:
                ...
                for vm in vcfa_vms:
                    ...
                    elif hasattr(vm, 'summary') and vm.summary and hasattr(vm.summary, 'config'):
                        num_cpus = vm.summary.config.numCpu
                    
                    if num_cpus == 32:
                        lsf.write_output(f'VCF Automation VM {vm.name} has 32 CPUs. Modifying to 24 CPUs with 8 cores per socket...')
                        if dry_run:
                            lsf.write_output(f'DRY-RUN: Would modify VM {vm.name} CPU count to 24 (8 cores per socket)')
                        else:
                            try:
                                spec = vim.vm.ConfigSpec()
                                spec.numCPUs = 24
                                spec.numCoresPerSocket = 8
                                task = vm.ReconfigVM_Task(spec=spec)
                                WaitForTask(task)
                                lsf.write_output(f'Successfully modified VM {vm.name} to 24 CPUs with 8 cores per socket')
                            except Exception as err:
                                lsf.write_output(f'ERROR: Failed to modify VM {vm.name} CPU configuration: {err}')
                    else:
                        lsf.write_output(f'VCF Automation VM {vm.name} has {num_cpus} CPUs (no modification needed)')
