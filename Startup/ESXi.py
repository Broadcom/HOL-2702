...
# Version 3.9 - 2026-08-26
# Author - Burke Azbill and HOL Core Team
# Verifies ESXi hosts are online and responsive
# v3.9: HOL-2702 specific - ensure each vsp-.* VM has at least 6 vCPUs and 12 GB (12288 MB) RAM before power-on.
...
def main(lsf=None, standalone=False, dry_run=False):
    ...
    ##=========================================================================
    ## CUSTOM - ESXi Advanced System Settings & VM CPU Topology / Resource Enforcement
    ##
    ...
    ##    (HOL-2701: cap at 24 vCPUs).
    ##
    ## 3. For each ESXi host, find any VM whose name starts with 'vsp-' (vsp-.*)
    ...
    ##    HOL-2702 specific: ensure each vsp-.* node has at least 6 CPU and 12 GB RAM
    ##    before power-on.
    ##
    ...
    if dry_run:
        lsf.write_output('Dry-run: skipping ESXi advanced settings and VM CPU topology / resource enforcement')
    elif not esx_hosts:
        ...
        lsf.write_output('Checking ESXi host advanced settings and VM CPU topology / resources...')
        try:
            import re
                ...
                for _attempt in range(MAX_HOST_RETRIES):
                    try:
                        ...
                        if not _vcfa_vms and not _vsp_vms:
                            lsf.write_output(f'  {_host}: no auto-platform-a* or vsp-.* VMs found')
                        ...
                        if _vsp_vms:
                            ...
                            for _vm in _vsp_vms:
                                ...
                                _cur_cores = _hw.numCoresPerSocket if _hw else None
                                _cur_num_cpu = _hw.numCPU if _hw else None
                                _cur_mem = _hw.memoryMB if _hw else None
                                _power = _vm.runtime.powerState

                                _mem_str = f', MemMB={_cur_mem} ({_cur_mem // 1024} GB)' if _cur_mem is not None else ''
                                lsf.write_output(
                                    f'    {_vm.name}: numCPU={_cur_num_cpu}{_mem_str}, '
                                    f'numCoresPerSocket={_cur_cores}, state={_power}'
                                )

                                # Determine target vCPU and memory count
                                _target_num_cpu = _cur_num_cpu
                                _target_mem = _cur_mem

                                # HOL-2702 specific: ensure at least 6 CPU and 12 GB (12288 MB) RAM
                                if lsf.lab_sku == 'HOL-2702':
                                    VSP_MIN_CPU = 6
                                    VSP_MIN_MEM_MB = 12 * 1024  # 12288 MB (12 GB)
                                    if _target_num_cpu is not None and _target_num_cpu < VSP_MIN_CPU:
                                        _target_num_cpu = VSP_MIN_CPU
                                    if _target_mem is not None and _target_mem < VSP_MIN_MEM_MB:
                                        _target_mem = VSP_MIN_MEM_MB
                                ...
                                _needs_cpu = (_cur_num_cpu != _target_num_cpu) if (_cur_num_cpu is not None and _target_num_cpu is not None) else False
                                _needs_mem = (_cur_mem != _target_mem) if (_cur_mem is not None and _target_mem is not None) else False
                                _needs_cores = (_cur_cores != _target_cores) if (_cur_cores is not None and _target_cores is not None) else False

                                if not _needs_cpu and not _needs_mem and not _needs_cores:
                                    _mem_info = f', MemMB={_cur_mem}' if _cur_mem is not None else ''
                                    lsf.write_output(
                                        f'    {_vm.name}: numCPU={_cur_num_cpu}{_mem_info}, '
                                        f'numCoresPerSocket={_cur_cores} - no changes needed'
                                    )
                                    continue
                                ...
                                # Powered off — safe to reconfigure CPU, memory, and topology in a single spec
                                _spec = vim.vm.ConfigSpec()
                                _reconfig_msg_parts = []
                                if _needs_cpu and _target_num_cpu is not None:
                                    _spec.numCPUs = _target_num_cpu
                                    _reconfig_msg_parts.append(f'numCPU: {_cur_num_cpu} -> {_target_num_cpu}')
                                if _needs_mem and _target_mem is not None:
                                    _spec.memoryMB = _target_mem
                                    _reconfig_msg_parts.append(
                                        f'memoryMB: {_cur_mem} -> {_target_mem} ({_target_mem // 1024} GB)'
                                    )
                                if _needs_cores and _target_cores is not None:
                                    _spec.numCoresPerSocket = _target_cores
                                    _reconfig_msg_parts.append(f'numCoresPerSocket: {_cur_cores} -> {_target_cores}')

                                _reconfig_msg = ', '.join(_reconfig_msg_parts)
                                lsf.write_output(f'    {_vm.name}: reconfiguring VM ({_reconfig_msg})')
                                try:
                                    _task = _vm.ReconfigVM_Task(spec=_spec)
                                    WaitForTask(_task)
                                    lsf.write_output(f'    {_vm.name}: VM reconfiguration complete')
                                    _host_vsp_updated += 1
                                except Exception as _reconfig_err:
                                    lsf.write_output(
                                        f'    {_vm.name}: VM reconfiguration FAILED: {_reconfig_err}'
                                    )
            ...
            _summary_parts = [
                ...
                f'{vsp_vms_found} vsp-.* VM(s) found ({vsp_vms_updated} updated)',
            ]
